...
router.route('/register/').post(function(req, res){
   ...
   if(req.body.username === undefined || req.body.password === undefined || req.body.password.length < 7){
      //respond with a failure here;
      res.status(500).json({error: "Error with request, password must be at least 7 characters, must include {username, password}"});
   }
   else{
                ...
                if(err){
                   ...
                   if(doc === null){
                      ...
                      if(hashedPassword === doc.password){
                         if(session[postedUsername] !== null & session[postedUsername] !== undefined){
                            res.status(200).json({token: session[postedUsername].token});
                         }else{
                             var token = crypto.createHash('md5').update(crypto.randomBytes(64).toString('hex')).digest('hex');
